-- ============================================================
-- Migration: fix orario task type=multiple in task_complete
-- Data: 2026-05-05
--
-- La versione precedente della funzione usava v_next_str con
-- 'T00:00:00' hardcodato per i task di tipo multiple, perdendo
-- l'orario impostato dall'utente alla creazione.
--
-- Fix: usare v_time_of_day (ricavato da start_date, come già
-- fatto per il tipo recurring) anche per multiple.
--
-- NOTA: task_complete è il punto unico di gestione dei completamenti.
-- Non aggiungere bypass client-side in tasks.html.
-- ============================================================
...
BEGIN
  ...
  -- mai modificato da task_complete). Usato per preservare l'ora su recurring e multiple.
  ...
  ELSIF v_task.type = 'multiple' THEN
    ...
    IF v_cur_idx IS NOT NULL AND v_cur_idx < array_length(v_dates, 1) THEN
      -- Preserva l'orario originale da start_date (stesso approccio di recurring)
