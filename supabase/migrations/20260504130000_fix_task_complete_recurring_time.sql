-- ============================================================
-- Migration: fix orario perde su task recurring dopo completamento
-- Data: 2026-05-04
--
-- Bug: per task di tipo 'recurring', task_next_recurring_date()
-- restituisce un tipo DATE (senza orario). Il cast date::timestamptz
-- imposta l'orario a 00:00:00, perdendo l'orario originale del task.
--
-- Fix: dopo aver calcolato v_next_date, si riaggancia l'orario
-- originale preso da next_occurrence_date (o start_date) come
-- intervallo da mezzanotte, e lo si somma alla nuova data.
-- ============================================================
...
BEGIN
  ...
  ELSIF v_task.type = 'recurring' THEN
    ...
    IF v_next_date IS NOT NULL THEN
      -- Preserva l'orario originale del task: calcola l'intervallo da mezzanotte
      -- (es. 09:30 → 9h 30m 0s) e lo aggiunge alla nuova data.
      -- Senza questo fix, date::timestamptz produce sempre 00:00:00.
      v_next_ts := v_next_date::timestamptz
                   + (
                       COALESCE(v_task.next_occurrence_date, v_task.start_date, now())
                       - date_trunc('day', COALESCE(v_task.next_occurrence_date, v_task.start_date, now()))
                     );
    END IF;
