-- Re-invia la notifica di test del picker categorie con navigazione ◀ ▶ (giro 2), stesso
-- pattern di 20260725150000: 3 transazioni finte nuove, per riprovare dopo l'installazione
-- dell'APK aggiornata (v1.4.0).
do $$
declare
  v_user_id      uuid;
  v_rule_id      uuid;
...
begin
  ...
  insert into ca_transactions (user_id, date, amount, currency, description, person_source, import_source)
    values (v_user_id, current_date, -9.99, 'EUR', 'TEST — cancellami (nav2 1)', 'unassigned', 'csv')
    returning id into v_tx1;
  insert into ca_transactions (user_id, date, amount, currency, description, person_source, import_source)
    values (v_user_id, current_date - 1, -25.00, 'EUR', 'TEST — cancellami (nav2 2)', 'unassigned', 'csv')
    returning id into v_tx2;
  insert into ca_transactions (user_id, date, amount, currency, description, person_source, import_source)
    values (v_user_id, current_date - 2, -3.50, 'EUR', 'TEST — cancellami (nav2 3)', 'unassigned', 'csv')
  ...
  v_transactions := jsonb_build_array(
    jsonb_build_object('id', v_tx1, 'description', 'TEST — cancellami (nav2 1)',
                        'amount', -9.99, 'currency', 'EUR', 'date', current_date::text),
    jsonb_build_object('id', v_tx2, 'description', 'TEST — cancellami (nav2 2)',
                        'amount', -25.00, 'currency', 'EUR', 'date', (current_date - 1)::text),
    jsonb_build_object('id', v_tx3, 'description', 'TEST — cancellami (nav2 3)',
  ...
  values (
    ...
    'TEST — categorizza 3 transazioni finte (nav ◀ ▶, giro 2)',
