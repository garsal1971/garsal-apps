-- ============================================================
-- Migration: fix v2 — orario recurring usa start_date, non next_occurrence_date
-- Data: 2026-05-04
--
-- Il fix precedente (20260504130000) usava COALESCE(next_occurrence_date, start_date)
-- come fonte dell'orario. Se next_occurrence_date era già a 00:00 (per task
-- completati con la versione buggy), il fix leggeva 00:00 e riscriveva 00:00.
--
-- Fix corretto: usare sempre start_date come fonte dell'orario,
-- perché start_date è impostato alla creazione e non viene mai modificato
-- da task_complete.
-- ============================================================
...
BEGIN
  ...
  -- Orario originale del task ricavato da start_date (impostato alla creazione,
  -- mai modificato da task_complete). Usato per preservare l'ora su recurring.
  ...
  ELSIF v_task.type = 'recurring' THEN
    ...
    IF v_next_date IS NOT NULL THEN
      -- Riaggancia l'orario originale da start_date (fonte certa: impostato
      -- dall'utente alla creazione e mai sovrascritto da task_complete).
      -- next_occurrence_date non è affidabile perché versioni precedenti
      -- scrivevano 00:00 dopo ogni completamento.
