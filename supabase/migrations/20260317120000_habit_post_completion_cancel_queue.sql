-- ============================================================
-- Aggiornamento habit_post_completion:
-- Cancella i promemoria pending dalla coda (cm_notification_queue)
-- per il giorno completato, sia dal click Telegram che dall'app.
-- ============================================================
...
BEGIN
  ...
  -- ── Cancella promemoria pending per questa giornata ───────────────────────
  -- Step 1: porta a 'cancelled' le entry pending
  UPDATE cm_notification_queue
  SET status = 'cancelled'
  WHERE app       = 'habits'
    AND entity_id = p_habit_id::text
    AND fire_at  >= (p_local_date || 'T00:00:00Z')::timestamptz
    AND fire_at  <= (p_local_date || 'T23:59:59Z')::timestamptz
    AND status    = 'pending';

  -- Step 2: elimina fisicamente le entry cancellate
