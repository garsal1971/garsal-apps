-- Re-invia la notifica di test del picker categorie (probabilmente l'APK v1.3.5 non era ancora
-- installato quando è arrivata la prima, o la riga era già passata a 'sent'/scaduta). Riusa le
-- transazioni fittizie della migration precedente (20260724200000) se esistono ancora e sono
-- senza categoria; altrimenti ne crea di nuove, stesso schema.
do $$
declare
  v_user_id      uuid;
  v_rule_id      uuid;
...
begin
  ...
  values (
    ...
    'TEST — categorizza 2 transazioni finte (rinvio)',
