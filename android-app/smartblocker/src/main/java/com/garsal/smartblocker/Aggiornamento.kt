 ...
 *
 * ⚠️ Gemello di `Aggiornamento.kt` dell'APK WebView (`com.garsalapps`) e di
 * `core/Aggiornamento.kt` dell'app nativa: stessa scheda `-latest.json` scritta
 * dal workflow accanto all'APK, stessa domanda, stessa risposta. Se cambia la
 * forma della scheda in un workflow, cambia in tutti e tre — e in
 * `mostraVersione()` di `comandi.html`, che legge le stesse sette chiavi.
 *
 ...
 *
 * ⚠️ Qui la scheda dipende dal **flavor**: `BuildConfig.RELEASE_BASE` vale
 * `SmartBlocker-latest` per Salvatore e `SmartBlockerTeresa-latest` per Teresa.
 * Sono due APK con due `applicationId` diversi: puntando tutt'e due alla stessa
 * scheda, il telefono di Teresa si vedrebbe offrire un pacchetto che non si
 * installa sopra al suo.
 */
object Rilascio {
    ...
    fun apk(versione: String): String =
        "$SITO/releases/${BuildConfig.RELEASE_BASE}.apk?v=$versione"
    ...
    suspend fun scheda(): Scheda = withContext(Dispatchers.IO) {
        ...
        val url = "$SITO/releases/${BuildConfig.RELEASE_BASE}.json?t=${System.currentTimeMillis()}"
...
/** Versione installata, letta dal **pacchetto** e non da `BuildConfig`: è quella
 *  vera, non quella che il codice credeva di essere. */
