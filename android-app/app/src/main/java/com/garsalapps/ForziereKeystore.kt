...
/**
 * Le 24 parole del Forziere avvolte nel Keystore di Android, per aprirlo con
 * l'impronta invece che con la passphrase — la «fase 2» del progetto.
 *
 ...
 * un'autenticazione fresca il `Cipher` non produce un byte. Fallisce CHIUSO, ed
 * è il motivo per cui questo file esiste separato.
 *
 ...
 * digita davanti a chiunque in autobus. E su API < 30 la coppia con un
 * `CryptoObject` non è nemmeno ammessa.
 *
...
object ForziereKeystore {
    ...
    // Gli esiti che tornano al JavaScript. Sono quattro e non due perché
    // «hai rinunciato» e «la registrazione non vale più» chiedono all'utente
    // due cose diverse, e un «errore» generico non gliele direbbe.
    const val OK = "ok"
    const val RINUNCIA = "rinuncia"
    ...
    /**
     * Il `Cipher` per avvolgere. La chiave si rifà da capo ogni volta: registrare
     * di nuovo **sostituisce**, non affianca, e una chiave vecchia lasciata lì
     * aprirebbe un blob che non c'è più.
     */
     ...
     * registrazione morta lasciata in giro è un pulsante che promette uno sblocco
     * e non lo dà.
     */
    ...
    fun svolgi(ctx: Context, c: Cipher): String {
        val salvato = prefs(ctx).getString(BLOB, null) ?: throw IllegalStateException("niente da aprire")
